...
ULI::ULI(unsigned _base) : base(_base) {}

ULI::ULI(unsigned int num, unsigned base) : base(base)
{
    do {
        arr.PushBack(num % 2);
        num >>= 1;
...
ULI::ULI(const char *str, unsigned _base, size_t _len) : base(_base)
{
    if (_len == 0)
        _len = strlen(str);
...
//     result = result + W[0] + W[1] << len/3 + W[2] << 2 * len/3 + W[3] << 3 *
//     len/3 + W[4] << 4 * len/3; return result;
// }
...
bool ULI::IsPrimary(unsigned method, unsigned k) const
{
    if (method > 4 || !method)
    ...
    case MILLERA_RABINA:
        return RabinMiller(k);
    case SOLOVEY_SHTRASSEN:
        return SoloveyShtrassen(k);
    }
    return false;
}
...
bool ULI::RabinMiller(unsigned k) const
{
    if (*this <= 3)
        return true;
    if (IsEven())
...
bool ULI::SoloveyShtrassen(unsigned k) const
{
    if ((*this) < 3)
        return true;
    if (IsEven())
...
bool ULI::Frobenius(unsigned k) const
{
    if (IsEven())
        return false;
}
